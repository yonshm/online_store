...
<div class="mb-3">
  <form method="GET" action="{{ route('product.index') }}" class="d-flex align-items-center">
    <select name="filter" id="select">
      <option value="-1" {{ old('filter', request('filter')) == '-1' ? 'selected' : '' }}>All Products</option>
      <option value="discounted" {{ old('filter', request('filter')) == 'discounted' ? 'selected' : '' }}>Discounted Products</option>
    </select>
    <button type="submit" class="btn btn-sm btn-outline-primary ms-2">Filtrer</button>
  </form>
</div>
<div class="row">
