    ...
    <div class="mb-3">
        <form method="GET" action="{{ route('product.index') }}" class="d-flex align-items-center">
            <select name="filter" id="select">
                <option value="-1" {{ old('filter', request('filter')) == '-1' ? 'selected' : '' }}>
                    {{ __('All Products') }}</option>
                <option value="discounted" {{ old('filter', request('filter')) == 'discounted' ? 'selected' : '' }}>
                    {{ __('Discounted Products') }}</option>
            </select>
                ...
                <div class="card">
                    ...
                    <div class="card-body text-center">
                        ...
                        @php
                            ...
                            $discountedPrice = method_exists($product, 'getDiscountedPrice')
